...
ALTER TABLE `users` ADD `age` INT NOT NULL AFTER `phone`, ADD `gender` VARCHAR(15) NOT NULL AFTER `age`, ADD `state` INT NOT NULL AFTER `gender`, ADD `city` INT NOT NULL AFTER `state`, ADD `user_type` INT NOT NULL COMMENT '1 for admin, 2 for hospital, 3 for doctors, 4 for patients' AFTER `city`;ALTER TABLE `users` ADD `age_type` VARCHAR(10) NOT NULL AFTER `age`;
...
CREATE TABLE `pt_medhist` (
  `mhis_id` int(11) AUTO_INCREMENT primary key,
  `mhis_uid` int(11) NOT NULL COMMENT 'patient user id',
  `mhis_sympt` text DEFAULT NULL,
  `mhis_treat` text DEFAULT NULL,
  `mhis_query` text DEFAULT NULL,
  `mhis_patho` text DEFAULT NULL COMMENT 'pathology report',
  `mhis_radio` text DEFAULT NULL COMMENT 'radiology report',
  `mhis_other` text DEFAULT NULL COMMENT 'other report',
  `reg_entrydt` datetime NOT NULL,
  `reg_status` int(11) NOT NULL DEFAULT '1' COMMENT '0 inactive 1 pending 2 Approve'
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
